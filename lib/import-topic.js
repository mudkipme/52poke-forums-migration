...
class ImportTopic {
  ...
  async importAll() {
    ...
    let finished = false;
    let start = 0;
    const indexes = await util.promisify(db.getSortedSetRevRange)('_import:_topics', 0, 0);
    if (indexes.length > 0) {
  ...
  async importTopic(row) {
    const imported = await util.promisify(db.isSortedSetMember)('_import:_topics', row.tid);
    if (imported) {
    ...
    const userMeta = await util.promisify(db.getObject)(`_import:_user:${row.authorid}`);
    if (!userMeta) {
    ...
    await util.promisify(db.setObject)(`_import:_topic:${row.tid}`, { tid: topicData.tid });
    await util.promisify(db.sortedSetAdd)('_import:_topics', topicData.tid, row.tid);
