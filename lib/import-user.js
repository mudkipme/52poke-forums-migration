...
class ImportUser {
  ...
  async importAll() {
    await this.processAll('importUser');
  }

  async updateSignatures() {
    await this.processAll('updateSignature');
  }

  async purgeNotifications() {
    await this.processAll('removeNotifications');
  }

  async updateTrophies() {
    await this.processAll('updateTrophy');
  }

  async processAll(method) {
    let current = 0;
    let limit = 100;
    let finished = false;
    ...
    while (!finished) {
      ...
      await this._processRows(rows, method);
      current += limit;
    }
  }

  async _processRows(rows, method) {
    for (let row of rows) {
      await this[method](row);
    }
  }

  async importUser(row) {
    const imported = await util.promisify(db.isSortedSetMember)('_import:_users', row.userid);
    if (imported) {
    ...
    const fields = {
      ...
      aboutme: bbcode.markdownStripReserves(row.desper),
      signature: bbcode.markdownStripReserves(row.signtext),
    ...
    if (fields.reputation > 0) {
      await util.promisify(db.sortedSetAdd)('users:reputation', fields.reputation, uid);
    }
    ...
    await util.promisify(db.setObject)(`_import:_user:${row.userid}`, { uid, username, userslug });
    await util.promisify(db.sortedSetAdd)('_import:_users', uid, row.userid);
  ...
  async updateSignature(row) {
    ...
    if (Object.keys(_.pickBy(fields)).length > 0) {
      await util.promisify(User.setUserFields)(userMeta.uid, _.pickBy(fields));
    }

    console.log(`Updated ${userMeta.username}.`);
  }

  async removeNotifications(row) {
    const userMeta = await util.promisify(db.getObject)(`_import:_user:${row.userid}`);
    if (!userMeta) {
      console.log(`${row.username} is not imported.`);
      return;
    ...
    await util.promisify(User.notifications.deleteAll)(userMeta.uid);
    console.log(`Processed ${userMeta.username}.`);
  }

  async updateTrophy(row) {
    if (!row.medals) {
      console.log(`Skip ${row.username}. (No trophies).`);
      return;
    }
    ...
    const medals = row.medals.split(',');
    await util.promisify(db.setAdd)('trophy-plugin:users', userMeta.uid);
    for (let medal of medals) {
      const trophy = this.config.trophyMap[medal];
      await util.promisify(db.setAdd)('trophy-plugin:user:' + userMeta.uid, trophy);
    }
    console.log(`Updated ${userMeta.username}.`);
  }
}
