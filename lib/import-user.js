...
class ImportUser {
  ...
  async updateSignature(row) {
    const userMeta = await util.promisify(db.getObject.bind(db))(`_import:_user:${row.userid}`);
    if (!userMeta) {
  ...
  async updateTrophy(row) {
    ...
    await util.promisify(db.setAdd.bind(db))('trophy-plugin:users', userMeta.uid);
    for (let medal of medals) {
      const trophy = this.config.trophyMap[medal];
      await util.promisify(db.setAdd.bind(db))('trophy-plugin:user:' + userMeta.uid, trophy);
    }
    console.log(`Updated ${userMeta.username}.`);
  }
}
