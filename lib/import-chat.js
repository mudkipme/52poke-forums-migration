...
class ImportChat {
  ...
  async importChat(row) {
    ...
    const imported = await util.promisify(db.isSortedSetMember.bind(db))('_import:_chats', row.id);
    if (imported) {
      console.log(`${row.id} was already imported.`);
      return;
    }
    
    const from = await util.promisify(db.getObject.bind(db))(`_import:_user:${row.blid}`);
    const to = await util.promisify(db.getObject.bind(db))(`_import:_user:${row.stid}`);
    ...
    await util.promisify(db.setObject.bind(db))(`_import:_chat:${row.id}`, { mid: message.mid, roomId });
    await util.promisify(db.sortedSetAdd.bind(db))('_import:_chats', message.mid, row.id);
