...
// Disable attribute support
const _parseParams = bbcode.Parser.prototype._parseParams;
bbcode.Parser.prototype._parseParams = function (token) {
  const index = token.indexOf('=');
  if (index > -1) {
    return [[token.substr(0, index), token.substr(index + 1)]];
  }
  return _parseParams.call(this, token);
};
...
class QuotedTag extends bbcode.BUILTIN.quote {
  constructor(renderer, settings) {
    super(renderer, settings);
  }

  _toHTML() {
    const pieces = ['<blockquote>'];
    const title = this.params['quoted'];
    if (title) {
      pieces.push('<h3>');
      pieces.push(title);
      pieces.push('</h3>');
...
parser.registerTag('quoted', QuotedTag);
...
exports.markdownStripReserves = function (str) {
  str = exports.markdownFromBBCode(str);
  return str.replace(/%%RESTRICTED_CONTENT%%/g, '')
    .replace(/%%UPLOAD%%(\d+)%%/g, '');
};
